	...
	(0, _createClass3.default)(SCR0704Controller, [{
		...
		value: function getEncCodes() {
			...
			this.httpService.get('encCd').then(function (data) {
				...
				_this3.encCd = records;
				if (!_.isEmpty(_this3.encCd)) {
					_this3.encCd.map(function (records) {
						defaultArray.push({
							id: records.encCd,
							text: records.encNm
						});
		...
		value: function _getDefaultMsgDetailGridColumns(msg) {
			...
			var columns = [{ field: 'msgSeq', caption: 'seq', size: '40px', frozen: true }, { field: 'fldEngNm', caption: this.text.fldEngNm, size: '2%', frozen: true, attr: "align=left",
				...
				render: function render(data, index, colIndex) {
					var dataTypeNm = w2ui[_this4.msgDetailOptions.name].getCellValue(index, colIndex);
					return _this4.codeService.getCodeValNm(_this4.codes.DATA_TYPE, dataTypeNm);
				}
				...
				render: function render(data, index, colIndex) {
					var privacyDscd = w2ui[_this4.msgDetailOptions.name].getCellValue(index, colIndex);
					return _this4.getMaskNm(privacyDscd);
				}
				...
				render: function render(data, index, colIndex) {
					var encCd = w2ui[_this4.msgDetailOptions.name].getCellValue(index, colIndex);
					return _this4.getEncNm(encCd);
				}
			}, {
				...
				render: function render(data, index, colIndex) {
					var paramTypeNm = w2ui[_this4.msgDetailOptions.name].getCellValue(index, colIndex);
					return _this4.codeService.getCodeValNm(_this4.codes.PARAM_TYPE, paramTypeNm);
				}
			}, {
				...
				render: function render(data, index, colIndex) {
					var alignNm = w2ui[_this4.msgDetailOptions.name].getCellValue(index, colIndex);
					return _this4.codeService.getCodeValNm(_this4.codes.ALIGN_CD, alignNm);
				}
			}, {
				...
				render: function render(data, index, colIndex) {
					var fillerVal = w2ui[_this4.msgDetailOptions.name].getCellValue(index, colIndex);
					return _this4.codeService.getCodeValNm(_this4.codes.FILLER_CD, fillerVal);
				}
			}, {
		...
		value: function getMsgDetailGridData() {
			...
			if (this.data.gridData) {
				...
				this.httpService.get('/msglayouts/' + encodedId).then(function (res) {
					_this5.displayMsgDetailGridData(res);
				});
			}
		}
	}, {
		...
		value: function displayMsgDetailGridData(res) {
			...
			this.msgDetailOptions.records = originRecords.map(function (record) {
				record.fldUnqId = record.fldUnqId ? record.fldUnqId : _this6.utilService.uniqueId('TEMP_LAYOUT_DD_DTO_ID');
				return record;
			});
		...
		value: function closeModal(isOk) {
			...
			setTimeout(function () {
				return w2ui[_this7.msgDetailOptions.name].destroy();
			});
		}
	}]);
