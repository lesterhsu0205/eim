webpackJsonp(["app\\views\\wrap\\manageFepInterface\\SCR0901.controller"],{
...
module.exports = __webpack_require__("85cc2026746c457aa840");


/***/ }),

/***/ "85cc2026746c457aa840":
/***/ (function(module, exports, __webpack_require__) {
...
var SCR0901Controller = function () {
	function SCR0901Controller($scope, $state, $compile, $timeout, popupService, httpService, utilService, gridService, codeService, metaService, userService, codes) {
		var _this = this;

		(0, _classCallCheck3.default)(this, SCR0901Controller);
	...
	(0, _createClass3.default)(SCR0901Controller, [{
		key: 'initPrevData',
		...
		value: function initGrid() {
			...
			this.inferfaceMainGrid = {
				...
				columns: [{ field: 'intrfcId', caption: this.text.intrfcId, size: '1.5%', sortable: true }, { field: 'intrfcNm', caption: this.text.intrfcNm, size: '2.5%', sortable: true, attr: 'align=left' }, { field: 'intrfcNmSub', caption: this.text.intrfcNmSub, size: '2.5%', sortable: true, attr: 'align=left' }, {
					...
					field: 'trxDscd', caption: this.text.trxDscd2, size: this.user.locale === 'en' ? '110px' : '0.5%', sortable: true,
					render: function render(data) {
						return _this5.codeService.getCodeValNm('TRAN_DSCD', data.trxDscd);
					}
				}, { field: 'sysCdS', caption: this.text.sysCdS, size: this.user.locale === 'en' ? '90px' : '0.5%', sortable: true }, { field: 'sysCdR', caption: this.text.sysCdR, size: this.user.locale === 'en' ? '95px' : '0.5%', sortable: true }, {
					field: 'syncAsyncDscd', caption: this.text.syncAsyncDscd, size: '0.5%', sortable: true,
					render: function render(data) {
						return _this5.codeService.getCodeValNm('SYNC_DSCD', data.syncAsyncDscd);
					} }, { field: 'msgTrnsfrmYn', caption: this.text.msgTrnsfrmYn, size: '0.5%', sortable: true }, { field: 'trxCd', caption: this.text.trxCd, size: '0.8%', sortable: true }, {
					caption: this.text.reqResMsgNo, size: this.user.locale === 'en' ? '210px' : '0.7%',
					render: function render(data) {
						var renderText = '';
						var rqstExtrnlMsgNo = data.rqstExtrnlMsgNo ? data.rqstExtrnlMsgNo : '';
						var rspsExtrnlMsgNo = data.rspsExtrnlMsgNo ? data.rspsExtrnlMsgNo : '';
			...
			this.interfaceDetailGrid = {
				...
				onEditField: function onEditField(e) {
					...
					if (_this5.detail.trxDscd == 'ONLINE' && e.column == 4 && record.srTypeCd == "SEND") {
						e.preventDefault();
					}
				},
		...
		value: function editIntrfcDetailGrid() {
			...
			}, { field: 'sysNm', caption: this.text.sysNm, size: '1.5%' }, { field: 'crgManNm', caption: '<i class="asterisk"></i>' + this.text.crgManNm, size: '1.5%', editable: { type: 'text' } }, { field: 'trxCd', caption: '<i class=""></i>' + this.text.trxCd, size: '1%', editable: { type: 'text' } }];
		...
		value: function getSndRecColumns() {
			...
			return [{
				...
				render: function render(data, index, colIndex) {
					...
					return _this7.codeService.getCodeValNm('MSG_TYPE', msgDscd);
				}
			}, {
				...
				render: function render(data, index, colIndex) {
					...
					return _this7.codeService.getCodeValNm('WORK_STATUS_CD', workStatusCd);
				}
			}, {
				...
				render: function render(data) {
					return '\n\t\t\t\t\t\t<button type="button" class="bw-btn bxd bxd-split-vertical" title="' + _this7.text.msgDiff + '" data-action="diff"></button>\n\t\t\t\t\t';
				}
			}, {
		...
		value: function initMsgTrgColumns() {
			...
			this.sendMsgMapTrgGrid.columns = [{ field: 'msgLayoutId', caption: this.text.msgLayoutId, size: '1.2%' }, { field: 'fldEngNm', caption: this.text.fldEngNm, size: '2%', attr: 'align=left' }, { field: 'fldKorNm', caption: this.text.fldKorNm, size: '1.3%', attr: 'align=left' }, { field: 'dataTypeNm', caption: this.text.dataTypeNm, size: '1%' }, { field: 'fldLvNo', caption: this.text.fldLvNo, size: '50px' }, { field: 'msgLen', caption: this.text.msgLen, size: '50px' }, {
				...
				render: function render(data, index, colIndex) {
					var mappingTypeCd = w2ui[_this8.sendMsgMapTrgGrid.name].getCellValue(index, colIndex);
					return _this8.codeService.getCodeValNm(_this8.codes.MAPPING_DSCD, mappingTypeCd);
				}
			}, this.sendMsgMapTrgSrcData];
			...
			this.rcvMsgMapTrgGrid.columns = [{ field: 'msgLayoutId', caption: this.text.msgLayoutId, size: '1.2%' }, { field: 'fldEngNm', caption: this.text.fldEngNm, size: '2%', attr: 'align=left' }, { field: 'fldKorNm', caption: this.text.fldKorNm, size: '1.3%', attr: 'align=left' }, { field: 'dataTypeNm', caption: this.text.dataTypeNm, size: '1%' }, { field: 'fldLvNo', caption: this.text.fldLvNo, size: '50px' }, { field: 'msgLen', caption: this.text.msgLen, size: '50px' }, {
				...
				editable: { type: 'select', items: this.gridService.getSelectItemsFromCodes(this.codes.MAPPING_DSCD) },
				render: function render(data, index, colIndex) {
					var mappingTypeCd = w2ui[_this8.rcvMsgMapTrgGrid.name].getCellValue(index, colIndex);
					return _this8.codeService.getCodeValNm(_this8.codes.MAPPING_DSCD, mappingTypeCd);
				}
			}, this.recvMsgMapTrgSrcData];
		}
		...
		value: function getIntrfccoms() {
			...
			httpService.get(url, this.searchParam.intrfccoms).then(function (res) {
				...
				if (res.isError) {
					_this9.popupService.detailAlert(_this9.$scope, res.data.message, res.data.stackTrace);
					return;
				}

				_this9.inferfaceMainGrid.records = records;
				_this9.inferfaceMainGrid.recordsCount = recordsCount;

				if (!_.isEmpty(_this9.inferfaceMainGrid.name)) {
					w2ui[_this9.inferfaceMainGrid.name].selectNone();
				}
		...
		value: function setIntrfGridData(id) {
			...
			httpService.get('/intrfccoms/' + id).then(function (data) {
				_this10.useSndReqToResLayoutGrid = false;
				_this10.useSndReqLayoutGrid = false;
				_this10.useSndResLayoutGrid = false;
				_this10.useRequestMsgScroll = true;
				_this10.useResponseMsgScroll = true;
				_this10.compulsionDeployYn = false;
				_this10.useRequestMsgMapping = false;
				_this10.useResponseMsgMapping = false;

				_this10.setIntrfAccordion(data, isCreateInterfaceId);
		...
		value: function setIntrfAccordion(data) {
			...
			var intrfcsrsysRecords = data.intrfcsrsysdtDto,
			    ...
			    intrfcroutinfodtDtoRecords = data.intrfcroutinfodtDto;
			...
			this.interfaceDetailGrid.records = intrfcsrsysRecords.map(function (intrfcsrsys) {
				return _this11.intfcDto(intrfcsrsys);
			});
			...
			intrfcsrsysRecords.map(function (intrfcsrsys) {
				if (intrfcsrsys.srTypeCd === 'SEND') {
					_this11.setSysCd('SEND', intrfcsrsys.sysCd, intrfcsrsys.sysNm);
				} else {
					_this11.setSysCd('RECEIVE', intrfcsrsys.sysCd, intrfcsrsys.sysNm);
				}
			});
		...
		key: 'changeTrxDscd',
		value: function changeTrxDscd(trxDscd) {
			if (trxDscd == 'ONLINE') {
				this.isOnline = true;
				this.isBatch = false;
		...
		key: 'setFEBOnlineDeployTarget',
		value: function setFEBOnlineDeployTarget() {
			var _this13 = this;

			this.httpService.post('/depolysyss/getlist', ['CBK', 'FEB']).then(function (data) {
				data.map(function (deploySys) {
					deploySys.deployUrl = deploySys.deploySysUrl;
					delete deploySys.deploySysUrl;
				});

				_this13.deployTargetSysGrid.records = data;
			});
		}
	}, {
		key: 'setFEBBatchDeployTarget',
		value: function setFEBBatchDeployTarget() {
			var _this14 = this;

			this.httpService.post('/depolysyss/getlist', ['CBK', 'FEB', 'EAI']).then(function (data) {
				data.map(function (deploySys) {
					deploySys.deployUrl = deploySys.deploySysUrl;
					delete deploySys.deploySysUrl;
				});
		...
		key: 'setFEPDeployTarget',
		value: function setFEPDeployTarget() {
			var _this15 = this;

			this.httpService.post('/depolysyss/getlist', ['CBK']).then(function (data) {
				data.map(function (deploySys) {
					deploySys.deployUrl = deploySys.deploySysUrl;
					delete deploySys.deploySysUrl;
				});
   ...
   */
		...
		value: function offEditMode() {
			var $forms = $('#searchWrap_detail,\n\t\t\t\t\t\t  #searchWrap_sndFileDesc,\n\t\t\t\t\t\t  #searchWrap_recvFileDesc,\n\t\t\t\t\t\t  #searchWrap_timeout,\n\t\t\t\t\t\t  #searchWrap_rcvReqMsg,\n\t\t\t\t\t\t  #searchWrap_rcvResMsg').find('input,textarea,select');
		...
		value: function openMsgLayoutPopup() {
			...
			this.popupService.openModal('SCR0502').then(function (data) {
				return _this19.searchParam.intrfccoms.msgLayoutId = data.msgLayoutId;
			}).catch(function () {});
		}
	}, {
		key: 'openInstCdPopup',
		value: function openInstCdPopup() {
			var _this20 = this;

			if (!this.isEdit) return;
			this.popupService.openModal('SCR1602').then(function (data) {
				_this20.detail.instCd = data.instCd;
				_this20.detail.instCdNm = data.instCdNm;
			}).catch(function () {});
		}
	}, {
		key: 'openInstCdPopup2',
		value: function openInstCdPopup2() {
			var _this21 = this;

			this.popupService.openModal('SCR1602').then(function (data) {
				return _this21.searchParam.intrfccoms.instCd = data.instCd;
			}).catch(function () {});
		}
	}, {
		key: 'openSystemPopup',
		value: function openSystemPopup(id) {
			var _this22 = this;

			this.popupService.openModal('SCR1302').then(function (data) {
				if (id == 'snd') {
					_this22.searchParam.intrfccoms.sysCdS = data.sysCd;
				} else if (id == 'rcv') {
					_this22.searchParam.intrfccoms.sysCdR = data.sysCd;
				}
			}).catch(function () {});
		}
	}, {
		...
		value: function openRegManPopup() {
			...
			this.popupService.openModal('SCR0102').then(function (user) {
				return _this23.searchParam.intrfccoms.regManId = user.userId;
			}).catch(function () {});
		}
	}, {
		...
		value: function openInterfaceIdCreatePopup() {
			...
			this.popupService.openModal('SCR0702', { intrfcTypeCd: this.intrfcTypeCd }).then(function (intrfc) {
				...
				if (detail.trxDscd == 'ONLINE') {
					_this24.isOnline = true;
					_this24.isBatch = false;
					detail.intrfcWayCd = 'APTOAP';
				} else if (detail.trxDscd == 'BATCH') {
					_this24.isOnline = false;
					_this24.isBatch = true;
					detail.intrfcWayCd = 'FILETOFILE';
				}

				_this24.editIntrfcDetailGrid();

				_this24.interfaceDetailGrid.records = [_this24.intfcDto({
				...
				_this24.setSysCd('SEND', sndSys.sysCd, sndSys.sysNm);
				_this24.setSysCd('RECEIVE', rcvSys.sysCd, rcvSys.sysNm);

				if (sndSys.sysCd == 'FEB' || rcvSys.sysCd == 'FEB') {
					if (detail.trxDscd == 'ONLINE') {
						_this24.setFEBOnlineDeployTarget();
					} else {
						_this24.setFEBBatchDeployTarget();
					}
				} else {
					_this24.setFEPDeployTarget();
				}
			}).catch(function () {});
		}
	}, {
		...
		value: function openDeploySystemPopup() {
			...
			this.popupService.openModal('SCR1202', { deploySysGrpCd: this.intrfcTypeCd, codes: { 'DEPLOY_SYS_DSCD': this.codes['DEPLOY_SYS_DSCD'] } }).then(function (deploySys) {
				if (!_.isEmpty(grid.get(deploySys.deploySysCd))) {
					_this25.openAlert(_this25.text.duplicateDeployTarget);
					return;
				}
			...
			this.popupService.openModal('SCR0502', {
				...
				msglayoutbsDtoList.map(function (msglayoutbsDto) {
					if (!_.isEmpty(grid.get(msglayoutbsDto.msgLayoutId))) {
						_this26.openAlert(_this26.text.duplicateMsg);
						return;
					}

					grid.add({
						intrfcId: _this26.detail.intrfcId,
						srTypeCd: srTypeCd,
					...
					var targetGrid = _this26._getTargetGridByOptionName(optionName);
		...
		value: function setGridSeq() {
			...
			this.sndMsgMapSrcGrid.records.map(function (record, index) {
				...
				_this27.sendMsgMapSrcMap[record.fldUnqId] = '[' + record.gridSeq + '] ' + record.fldUnqId;
			});

			this.sendMsgMapTrgSrcData.render = function (data) {
				if (data.mappingTypeCd === 'PROPT') {
					return _this27.sendMsgMapSrcMap[data.srcData];
				} else {
					return data.srcData;
				}
			};
			...
			this.rcvMsgMapSrcGrid.records.map(function (record, index) {
				record.gridSeq = index + 1;
				_this27.recvMsgMapSrcMap[record.fldUnqId] = '[' + record.gridSeq + '] ' + record.fldUnqId;
			});

			this.recvMsgMapTrgSrcData.render = function (data) {
				if (data.mappingTypeCd === 'PROPT') {
					return _this27.recvMsgMapSrcMap[data.srcData];
				} else {
					return data.srcData;
				}
			};
		...
		value: function deployInterface(event) {
			...
			this.popupService.simpleConfirm(this.$scope, this.text.confirmDeploy, function () {
				return _this28._deployInterface();
			});
		}
	}, {
		key: 'isRowMetaValid',
		...
		value: function _deployInterface() {
			...
			this.httpService.post('/intrfccoms/deploy', param).then(function (res) {
				...
				_this29.setDeployResult(res.intrfcDeployResponse);

				var url = '/intrfccoms/deployhistorys?intrfcId=' + _this29.detail.intrfcId;
				_this29.httpService.get(url).then(function (data) {
					_this29.refHistoryGrid.records = data.intrfcdeployhisthsOutList.map(function (v) {
						v.id = v.deploySysCd + v.deployVersion;
						return v;
					});
				});
				...
				var alertText = _this29.text.completeDeploy;

				if (successCnt !== 0 && filterResponse.length === successCnt) {
					alertText += '<br/><span class="chr-c-green">' + _this29.text.successDeploy + '</span>';
				} else if (failCnt !== 0) {
					alertText += '<br/><span class="chr-c-orange">' + _this29.text.failDeploy + '</span>';
				}

				_this29.openAlert(alertText);
			}).finally(function () {
				_this29.popupService.closeLoadingBar();
			});
		}
		...
		value: function redeployInterface(intrfcdeployhist) {
			...
			this.httpService.post('/intrfccoms/redeploy', intrfcdeployhistClone).then(function (res) {
				...
				var url = '/intrfccoms/deployhistorys?intrfcId=' + _this30.detail.intrfcId;
				_this30.httpService.get(url).then(function (data) {
					_this30.refHistoryGrid.records = data.intrfcdeployhisthsOutList.map(function (v) {
						v.id = v.deploySysCd + v.deployVersion;
						return v;
					});
				});
				...
				_this30.popupService.closeLoadingBar();
				_this30.openAlert(_this30.text.completeRedeploy);
			}).finally(function () {
				_this30.popupService.closeLoadingBar();
			});
		}
		...
		value: function save() {
			...
			var sendMsgMapTrgGrid = _w2ui[this.sendMsgMapTrgGrid.name];
			var rcvMsgMapTrgGrid = _w2ui[this.rcvMsgMapTrgGrid.name];

			sendMsgMapTrgGrid.save();
			rcvMsgMapTrgGrid.save();

			var sendMsgMapTrgGridRecords = sendMsgMapTrgGrid.records;
			var rcvMsgMapTrgGridRecords = rcvMsgMapTrgGrid.records;
			...
			requestPromise.then(function (res) {
				_this31.popupService.closeLoadingBar();

				if (res.isError) {
					_this31.popupService.detailAlert(_this31.$scope, res.data.message, res.data.stackTrace, res.data.parameters);
					return;
				}

				_this31.getIntrfccoms();
				if (_this31.isAdd) {
					_this31.setIntrfGridData(res.intrfcId);
				} else {
					_this31.setIntrfGridData(detail.intrfcId);
				}

				_this31.openAlert(bxMsg('common.saved'));
				_this31.offEditMode();
				_this31.detail.msgLayoutTranYn = 'N';
			});
		...
		value: function _checkValid() {
			...
			var sndReqLayoutGridRecords = w2ui[this.sndReqLayoutGrid.name].records;
			var sndResLayoutGridRecords = w2ui[this.sndResLayoutGrid.name].records;
			var rcvReqLayoutGridRecords = w2ui[this.rcvReqLayoutGrid.name].records;
			var rcvResLayoutGridRecords = w2ui[this.rcvResLayoutGrid.name].records;

			if (sndReqLayoutGridRecords.length > 1 || rcvReqLayoutGridRecords.length > 1) {
				if (_.isEmpty(intrfccoms.fepDto.reqWrapperDtoNm)) {
					this.openAlert(this.text.emptyReqWrapperDtoNm);
					return false;
				}
			}

			if (sndResLayoutGridRecords.length > 1 || rcvResLayoutGridRecords.length > 1) {
				if (_.isEmpty(intrfccoms.fepDto.resWrapperDtoNm)) {
					this.openAlert(this.text.emptyResWrapperDtoNm);
					return false;
				}
			}
		...
		value: function excelExport(id, multiDownload, isFirstFile, isLastFile) {
			...
			this.httpService.downloadFile('/intrfccoms/excelexport', data).then(function (res) {
				if (res.isError) {
					...
					_this32.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
				if (multiDownload) {
					isLastFile && _this32.popupService.closeLoadingBar();
				} else {
					_this32.popupService.closeLoadingBar();
				}
			});
		}
	}, {
		...
		value: function openInterfacePopup() {
			...
			this.popupService.openModal('SCR0902', { intrfcTypeCd: this.intrfcTypeCd, codes: this.codes }).then(function (intrfccom) {
				return _this33.setIntrfGridData(intrfccom.intrfcId, true);
			}).catch(function () {});
		}
	}, {
		...
		value: function _createInterFaceId() {
			...
			this.httpService.post('/intrfccoms/intrfcidcreate', requestBody).then(function (res) {
				...
				_this34.detail.intrfcId = res.intrfcId;
				_this34.detail.workStatusCd = 'WORKING';
				_this34.refHistoryGrid.records = [];
			});
		}
	}, {
		key: 'refreshGrid',
		...
		key: 'getInstCd',
		value: function getInstCd(intrfccoms) {
			var txt = '';
			var instCd = void 0;
			var instCdNm = void 0;
		...
		value: function xlsExport() {
			...
			selections.map(function (id, idx) {
				setTimeout(function () {
					_this35.excelExport(id, true, idx === 0, selectionLength === idx + 1);
				}, idx * 500);
			});
		}
	}, {
		...
		value: function exportDefinition() {
			...
			this.httpService.downloadFile(url, this.searchParam.intrfccoms, 'get').then(function (res) {
				if (res.isError) {
					_this36.popupService.detailAlert(_this36.$scope, res.data.message, res.data.stackTrace);
				} else {
					var header = decodeURIComponent(res.headers('Content-Disposition'));
					var fileName = header.split("=")[1].replace(/\"/gi, '');
					console.log(fileName);
					...
					var blob = new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
					_this36.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
				_this36.popupService.closeLoadingBar();
			});
		}
	}, {
		key: 'stopPropagation',
	...
	return SCR0901Controller;
}();

(0, _angular.module)(_app2.default.name).controller('SCR0901Controller', SCR0901Controller);

/***/ })

},[52]);
//# sourceMappingURL=SCR0901.controller.js.map