webpackJsonp(["app\\views\\wrap\\manageMciInterfaceDetail\\SCR0701.controller"],{
...
module.exports = __webpack_require__("d1d58b870678c828e105");


/***/ }),

/***/ "d1d58b870678c828e105":
/***/ (function(module, exports, __webpack_require__) {
...
var SCR0701Controller2 = function () {
	function SCR0701Controller2($scope, $state, $compile, $timeout, popupService, httpService, utilService, gridService, codeService, metaService, userService, codes) {
		var _this = this;

		(0, _classCallCheck3.default)(this, SCR0701Controller2);
	...
	(0, _createClass3.default)(SCR0701Controller2, [{
		key: 'initPrevData',
		value: function initPrevData() {
			...
			if (!_.isEmpty(param)) {
				...
				if (param.data) {
					...
					var data = param.data;
					var intrfcId = data.intrfcId;
					var deployVersion = data.deployVersion;
					var url = '/intrfccoms/deploy/' + intrfcId + '?intrfcId=' + intrfcId + '&deployVersion=' + deployVersion;

					this.httpService.get(url).then(function (data) {
						_this2.setIntrfAccordion(data);
						_this2._openIntrfaceDetailZabara();
						_this2.offEditMode();
					});
				}
			} else {
		...
		value: function editIntrfcDetailGrid() {
			...
			}, { field: 'sysNm', caption: this.text.sysNm, size: '1.5%' }, { field: 'crgManNm', caption: '<i class="asterisk"></i>' + this.text.crgManNm, size: '1.5%', editable: { type: 'text' } }, { field: 'trxCd', caption: '<i class="asterisk"></i>' + this.text.trxCd, size: '1.5%', editable: { type: 'text' } }];
		}
	}, {
		...
		value: function setDefaultDeployTarget() {
			...
			this.httpService.post('/depolysyss/getlist', ['MCA', 'MCI_E', 'SIM', 'RSH', 'E2E']).then(function (data) {
			...
			switch (optionName) {
				case 'sndReqLayoutGrid':
					...
					this.rcvReqLayoutGrid.records = grid.records.map(function (record) {
						var clone = _this14.utilService.clone(record);
		...
		value: function mappingConfirm(type, isMapping) {
			...
			setTimeout(function () {
				...
				var txt = isMapping ? _this15.text.completeMapping : _this15.text.completeMappingConfirm;
				var cnt = diffRcords.length;
				cnt !== 0 && (txt += '<br> ' + _this15.text.confirmError + ' <span class="chr-c-orange">' + cnt + '</span>' + _this15.text.cnt);

				_this15.popupService.simpleAlert(_this15.$scope, txt);
			}, 300);
		}
	}, {
		key: '_getDiffRecords',
		...
		value: function searchApplicationCodes() {
			...
			this.popupService.openModal('SCR1402', { limitLvCd: 0 }).then(function (code) {
				_this16.searchParam.intrfccoms.lv1Cd = code.appCd;
			}).catch(function () {});
		}
	}, {
		key: 'openRegManPopup',
		value: function openRegManPopup() {
			...
			this.popupService.openModal('SCR0102').then(function (user) {
				return _this17.searchParam.intrfccoms.regManId = user.userId;
			}).catch(function () {});
		}
	}, {
		key: 'openSysCdSPopup',
		value: function openSysCdSPopup() {
			...
			this.popupService.openModal('SCR1302').then(function (sys) {
				return _this18.searchParam.intrfccoms.sysCdS = sys.sysCd;
			}).catch(function () {});
		}
	}, {
		key: 'openSysCdRPopup',
		value: function openSysCdRPopup() {
			...
			this.popupService.openModal('SCR1302').then(function (sys) {
				return _this19.searchParam.intrfccoms.sysCdR = sys.sysCd;
			}).catch(function () {});
		}
	}, {
		...
		value: function openInterfaceIdCreatePopup() {
			...
			this.popupService.openModal('SCR0702', { intrfcTypeCd: this.intrfcTypeCd }).then(function (intrfc) {
				...
				_this20.interfaceDetailGrid.records = [_this20.intfcDto({
				...
				_this20.setSysCd('SEND', sndSys.sysCd, sndSys.sysNm);
				_this20.setSysCd('RECEIVE', rcvSys.sysCd, rcvSys.sysNm);
			}).catch(function () {});
		}
	}, {
		...
		value: function openDeploySystemPopup() {
			...
			this.popupService.openModal('SCR1202', { deploySysGrpCd: this.intrfcTypeCd, codes: this.codes }).then(function (deploySys) {
				if (!_.isEmpty(grid.get(deploySys.deploySysCd))) {
					_this21.openAlert(_this21.text.duplicateDeployTarget);
					return;
				}
			...
			this.popupService.openModal('SCR0502', {
				...
				msglayoutbsDtoList.map(function (msglayoutbsDto) {
					if (!_.isEmpty(grid.get(msglayoutbsDto.msgLayoutId))) {
						_this22.openAlert(_this22.text.duplicateMsg);
						return;
					}

					grid.add({
						intrfcId: _this22.detail.intrfcId,
						srTypeCd: srTypeCd,
					...
					var targetGrid = _this22._getTargetGridByOptionName(optionName);
		...
		value: function setGridSeq() {
			...
			this.sndMsgMapSrcGrid.records.map(function (record, index) {
				...
				_this23.sendMsgMapSrcMap[record.fldUnqId] = '[' + record.gridSeq + '] ' + record.fldUnqId;
			});

			this.sendMsgMapTrgSrcData.render = function (data) {
				if (data.mappingTypeCd === 'PROPT') {
					return _this23.sendMsgMapSrcMap[data.srcData];
				} else {
					return data.srcData;
				}
			};
			...
			this.rcvMsgMapSrcGrid.records.map(function (record, index) {
				record.gridSeq = index + 1;
				_this23.recvMsgMapSrcMap[record.fldUnqId] = '[' + record.gridSeq + '] ' + record.fldUnqId;
			});

			this.recvMsgMapTrgSrcData.render = function (data) {
				if (data.mappingTypeCd === 'PROPT') {
					return _this23.recvMsgMapSrcMap[data.srcData];
				} else {
					return data.srcData;
				}
			};
		...
		value: function deployInterface(event) {
			...
			this.popupService.simpleConfirm(this.$scope, this.text.confirmDeploy, function () {
				return _this24._deployInterface();
			});
		}
	}, {
		key: 'isRowMetaValid',
		...
		value: function _deployInterface() {
			...
			this.httpService.post('/intrfccoms/deploy', param).then(function (res) {
				if (res.isError) {
					_this25.popupService.detailAlert(_this25.$scope, res.data.message, res.data.stackTrace, res.data.parameters);
					return;
				}

				_this25.setDeployResult(res.intrfcDeployResponse);

				var url = '/intrfccoms/deployhistorys?intrfcId=' + _this25.detail.intrfcId;
				_this25.httpService.get(url).then(function (data) {
					_this25.refHistoryGrid.records = data.intrfcdeployhisthsOutList.map(function (v) {
						v.id = v.deploySysCd + v.deployVersion;
						return v;
					});
				});
				...
				_this25.popupService.closeLoadingBar();

				var alertText = _this25.text.completeDeploy;

				if (successCnt !== 0 && filterResponse.length === successCnt) {
					alertText += '<br/><span class="chr-c-green">' + _this25.text.successDeploy + '</span>';
				} else if (failCnt !== 0) {
					alertText += '<br/><span class="chr-c-orange">' + _this25.text.failDeploy + '</span>';
				}

				_this25.openAlert(alertText);
			}).finally(function () {
				_this25.popupService.closeLoadingBar();
			});
		}
		...
		value: function redeployInterface(intrfcdeployhist) {
			...
			this.httpService.post('/intrfccoms/redeploy', intrfcdeployhistClone).then(function (res) {
				_this26.popupService.closeLoadingBar();

				if (res.isError) {
					_this26.popupService.detailAlert(_this26.$scope, res.data.message, res.data.stackTrace);
					return;
				}

				var url = '/intrfccoms/deployhistorys?intrfcId=' + _this26.detail.intrfcId;
				_this26.httpService.get(url).then(function (data) {
					_this26.refHistoryGrid.records = data.intrfcdeployhisthsOutList.map(function (v) {
						v.id = v.deploySysCd + v.deployVersion;
						return v;
					});
				});
				...
				_this26.popupService.closeLoadingBar();
				_this26.openAlert(_this26.text.completeRedeploy);
			}).finally(function () {
				_this26.popupService.closeLoadingBar();
			});
		}
		...
		value: function save() {
			...
			requestPromise.then(function (res) {
				...
				_this27.getIntrfccoms();
				if (_this27.isAdd) {
					_this27.setIntrfGridData(res.intrfcId);
				} else {
					_this27.setIntrfGridData(detail.intrfcId);
				}

				_this27.openAlert(bxMsg('common.saved'));
				_this27.offEditMode();
				_this27.detail.msgLayoutTranYn = 'N';
			});
		...
		value: function excelExport(id, multiDownload, isFirstFile, isLastFile) {
			...
			this.httpService.downloadFile('/intrfccoms/excelexport', data).then(function (res) {
				if (res.isError) {
					...
					_this28.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
				if (multiDownload) {
					isLastFile && _this28.popupService.closeLoadingBar();
				} else {
		...
		value: function openInterfacePopup() {
			...
			this.popupService.openModal('SCR0902', { intrfcTypeCd: this.intrfcTypeCd, codes: this.codes }).then(function (intrfccom) {
				return _this29.setIntrfGridData(intrfccom.intrfcId, true);
			}).catch(function () {});
		}
	}, {
		...
		value: function _createInterFaceId() {
			...
			this.httpService.post('/intrfccoms/intrfcidcreate', requestBody).then(function (res) {
				...
				_this30.detail.intrfcId = res.intrfcId;
				_this30.detail.workStatusCd = 'WORKING';
				_this30.refHistoryGrid.records = [];
			});
		}
	}, {
		key: 'refreshGrid',
		...
		value: function xlsExport() {
			...
			selections.map(function (id, idx) {
				setTimeout(function () {
					_this31.excelExport(id, true, idx === 0, selectionLength === idx + 1);
				}, idx * 500);
			});
		}
	}, {
		...
		value: function importDefinition() {
			...
			this.httpService.uploadFileList('/intrfccoms/import/definition', { name: 'intrfcFile', file: fileList }, { intrfcTypeCd: this.intrfcTypeCd }).then(function (res) {
				...
				_this33.popupService.closeLoadingBar();
			});
		}
	}, {
		key: 'stopPropagation',
	...
	return SCR0701Controller2;
}();

(0, _angular.module)(_app2.default.name).controller('SCR0701Controller2', SCR0701Controller2);
