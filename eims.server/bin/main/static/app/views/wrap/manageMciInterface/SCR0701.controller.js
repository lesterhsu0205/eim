webpackJsonp(["app\\views\\wrap\\manageMciInterface\\SCR0701.controller"],{

/***/ "02f34c1db395e965c161":
...
var SCR0701Controller = function () {
	function SCR0701Controller($scope, $state, $compile, $timeout, popupService, httpService, utilService, gridService, codeService, metaService, userService, codes) {
		var _this = this;

		(0, _classCallCheck3.default)(this, SCR0701Controller);
	...
	(0, _createClass3.default)(SCR0701Controller, [{
		key: 'initPrevData',
		...
		value: function initGrid() {
			...
			this.inferfaceMainGrid = {
				...
				columns: [{ field: 'intrfcId', caption: this.text.intrfcId, size: '1.5%', sortable: true }, { field: 'intrfcNm', caption: this.text.intrfcNm, size: '2.5%', sortable: true, attr: 'align=left' }, { field: 'intrfcNmSub', caption: this.text.intrfcNmSub, size: '2.5%', sortable: true, attr: 'align=left' }, {
				...
				}, { field: 'sysCdS', caption: this.text.sysCdS, size: '0.5%', sortable: true }, { field: 'sysCdR', caption: this.text.sysCdR, size: '0.5%', sortable: true }, {
					...
					} }, { field: 'msgTrnsfrmYn', caption: this.text.msgTrnsfrmYn, size: '0.5%', sortable: true }, { field: 'rspsYn', caption: this.text.rspsYn, size: '0.5%', sortable: true }, { field: 'trxTypeDscd', caption: this.text.trxTypeDscd, size: '0.8%', sortable: true,
					render: function render(data) {
						return _this5.codeService.getCodeValNm('TRX_TYPE_DSCD', data.trxTypeDscd);
					} }, { field: 'viewId', caption: this.text.viewId, size: '0.8%', sortable: true }, { field: 'trxCd', caption: this.text.trxCd, size: '0.8%', sortable: true }, { field: 'viewId', caption: this.text.viewId, size: '0.8%', sortable: true }, { field: 'regManId', caption: this.text.regManId, size: '0.7%', sortable: true }, {
						...
						switch ($(eTarget).attr('data-action')) {
							...
							case 'redeploy':
								_this5.popupService.simpleConfirm(_this5.$scope, _this5.text.confirmRedeploy, function () {
									return _this5.redeployInterface(grid.get(recid));
								});
								break;
						}
					}
		...
		value: function editIntrfcDetailGrid() {
			...
			}, { field: 'sysNm', caption: this.text.sysNm, size: '1.5%' }, { field: 'crgManNm', caption: '<i class="asterisk"></i>' + this.text.crgManNm, size: '1.5%', editable: { type: 'text' } }, { field: 'trxCd', caption: '<i class=""></i>' + this.text.trxCd, size: '1.5%', editable: { type: 'text' } }];
		}
	}, {
		...
		value: function getSndRecColumns() {
			...
			return [{ field: 'rqstRspsSeq', caption: this.text.seq, size: '60px', render: function render(data, index) {
					return index + 1;
		...
		value: function setDefaultDeployTarget() {
			...
			this.httpService.post('/depolysyss/getlist', ['MCA', 'CBK', 'SUI']).then(function (data) {
		...
		value: function setCssDeployTarget() {
			...
			this.httpService.post('/depolysyss/getlist', ['MCA', 'CBK', 'SUI', 'CSS']).then(function (data) {
				data.map(function (deploySys) {
					deploySys.deployUrl = deploySys.deploySysUrl;
					delete deploySys.deploySysUrl;
				});
			...
			switch (optionName) {
				case 'sndReqLayoutGrid':
					...
					this.rcvReqLayoutGrid.records = grid.records.map(function (record) {
						var clone = _this15.utilService.clone(record);
   ...
   */
		...
		value: function mappingConfirm(type, isMapping) {
			...
			setTimeout(function () {
				...
				var txt = isMapping ? _this16.text.completeMapping : _this16.text.completeMappingConfirm;
				var cnt = diffRcords.length;
				cnt !== 0 && (txt += '<br> ' + _this16.text.confirmError + ' <span class="chr-c-orange">' + cnt + '</span>' + _this16.text.cnt);

				_this16.popupService.simpleAlert(_this16.$scope, txt);
			}, 300);
		}
	}, {
		key: '_getDiffRecords',
		...
		value: function searchApplicationCodes() {
			...
			this.popupService.openModal('SCR1402', { limitLvCd: 0 }).then(function (code) {
				_this17.searchParam.intrfccoms.lv1Cd = code.appCd;
			}).catch(function () {});
		}
	}, {
		key: 'openRegManPopup',
		value: function openRegManPopup() {
			...
			this.popupService.openModal('SCR0102').then(function (user) {
				return _this18.searchParam.intrfccoms.regManId = user.userId;
			}).catch(function () {});
		}
	}, {
		key: 'openSysCdSPopup',
		value: function openSysCdSPopup() {
			...
			this.popupService.openModal('SCR1302').then(function (sys) {
				return _this19.searchParam.intrfccoms.sysCdS = sys.sysCd;
			}).catch(function () {});
		}
	}, {
		key: 'openSysCdRPopup',
		value: function openSysCdRPopup() {
			...
			this.popupService.openModal('SCR1302').then(function (sys) {
				return _this20.searchParam.intrfccoms.sysCdR = sys.sysCd;
			}).catch(function () {});
		}
	}, {
		...
		value: function openInterfaceIdCreatePopup() {
			...
			this.popupService.openModal('SCR0702', { intrfcTypeCd: this.intrfcTypeCd }).then(function (intrfc) {
				...
				_this21.interfaceDetailGrid.records = [_this21.intfcDto({
				...
				_this21.setSysCd('SEND', sndSys.sysCd, sndSys.sysNm);
				_this21.setSysCd('RECEIVE', rcvSys.sysCd, rcvSys.sysNm);

				if (sndSys.sysCd == 'CSS' || rcvSys.sysCd == 'CSS') {
					_this21.setCssDeployTarget();
				} else {
					_this21.setDefaultDeployTarget();
				}
			}).catch(function () {});
		}
	}, {
		...
		value: function openDeploySystemPopup() {
			...
			this.popupService.openModal('SCR1202', { deploySysGrpCd: this.intrfcTypeCd, codes: this.codes }).then(function (deploySys) {
				if (!_.isEmpty(grid.get(deploySys.deploySysCd))) {
					_this22.openAlert(_this22.text.duplicateDeployTarget);
					return;
				}
		...
		value: function addMsgLayoutToSndRcvMsg(optionName) {
			...
			if (!this.isEdit) return;
			var grid = w2ui[this[optionName].name];
			var sndReqLayoutGrid = w2ui[this['sndReqLayoutGrid'].name];
			var sndResLayoutGrid = w2ui[this['sndResLayoutGrid'].name];
			...
			this.popupService.openModal('SCR0502', {
				...
				msglayoutbsDtoList.map(function (msglayoutbsDto) {
					if (!_.isEmpty(grid.get(msglayoutbsDto.msgLayoutId))) {
						_this23.openAlert(_this23.text.duplicateMsg);
						return;
					}
					if (optionName === 'sndReqLayoutGrid') {
						if (!_.isEmpty(sndResLayoutGrid.get(msglayoutbsDto.msgLayoutId))) {
							_this23.openAlert(_this23.text.systemDuplicateMsg);
							return;
						}
					} else if (optionName === 'sndResLayoutGrid') {
						if (!_.isEmpty(sndReqLayoutGrid.get(msglayoutbsDto.msgLayoutId))) {
							_this23.openAlert(_this23.text.systemDuplicateMsg);
							return;
						}
					}
					grid.add({
						intrfcId: _this23.detail.intrfcId,
						srTypeCd: srTypeCd,
					...
					var targetGrid = _this23._getTargetGridByOptionName(optionName);
		...
		value: function setGridSeq() {
			...
			this.sndMsgMapSrcGrid.records.map(function (record, index) {
				...
				_this24.sendMsgMapSrcMap[record.fldUnqId] = '[' + record.gridSeq + '] ' + record.fldUnqId;
			});

			this.sendMsgMapTrgSrcData.render = function (data) {
				if (data.mappingTypeCd === 'PROPT') {
					return _this24.sendMsgMapSrcMap[data.srcData];
				} else {
					return data.srcData;
				}
			};
			...
			this.rcvMsgMapSrcGrid.records.map(function (record, index) {
				record.gridSeq = index + 1;
				_this24.recvMsgMapSrcMap[record.fldUnqId] = '[' + record.gridSeq + '] ' + record.fldUnqId;
			});

			this.recvMsgMapTrgSrcData.render = function (data) {
				if (data.mappingTypeCd === 'PROPT') {
					return _this24.recvMsgMapSrcMap[data.srcData];
				} else {
					return data.srcData;
				}
			};
		...
		value: function deployInterface(event) {
			...
			this.popupService.simpleConfirm(this.$scope, this.text.confirmDeploy, function () {
				return _this25._deployInterface();
			});
		}
	}, {
		key: 'deployAllInterface',
		value: function deployAllInterface(event) {
			var _this26 = this;

			this.popupService.simpleConfirm(this.$scope, this.text.confirmAllDeploy, function () {
				return _this26._deployAllInterface();
			});
		}
	}, {
		key: 'isRowMetaValid',
		...
		key: '_deployAllInterface',
		value: function _deployAllInterface() {
			var _this27 = this;

			this.popupService.showLoadingBar(this.$scope);
			this.httpService.post('/intrfccoms/deployAll').then(function (res) {
				if (res.isError) {
		...
		value: function _deployInterface() {
			...
			this.httpService.post('/intrfccoms/deploy', param).then(function (res) {
				if (res.isError) {
					_this28.popupService.detailAlert(_this28.$scope, res.data.message, res.data.stackTrace, res.data.parameters);
					return;
				}

				_this28.setDeployResult(res.intrfcDeployResponse);
				...
				var alertText = _this28.text.completeDeploy;

				if (successCnt !== 0 && filterResponse.length === successCnt) {
					alertText += '<br/><span class="chr-c-green">' + _this28.text.successDeploy + '</span>';
				} else if (failCnt !== 0) {
					alertText += '<br/><span class="chr-c-orange">' + _this28.text.failDeploy + '</span>';
				}

				_this28.openAlert(alertText);
			}).finally(function () {
				_this28.popupService.closeLoadingBar();
			});
		}
		...
		value: function redeployInterface(intrfcdeployhist) {
			...
			this.httpService.post('/intrfccoms/redeploy', intrfcdeployhistClone).then(function (res) {
				...
				if (res.isError) {
					_this29.popupService.detailAlert(_this29.$scope, res.data.message, res.data.stackTrace);
					return;
				}
				...
				_this29.popupService.closeLoadingBar();
				_this29.openAlert(_this29.text.completeRedeploy);
			}).finally(function () {
				_this29.popupService.closeLoadingBar();
			});
		}
		...
		value: function save() {
			...
			requestPromise.then(function (res) {
				...
				if (res.isError) {
					_this30.popupService.detailAlert(_this30.$scope, res.data.message, res.data.stackTrace, res.data.parameters);
					return;
				}

				_this30.getIntrfccoms();
				if (_this30.isAdd) {
					_this30.setIntrfGridData(res.intrfcId);
				} else {
					_this30.setIntrfGridData(detail.intrfcId);
				}

				_this30.openAlert(bxMsg('common.saved'));
				_this30.offEditMode();
				_this30.detail.msgLayoutTranYn = 'N';
			});
		...
		value: function _checkValid() {
			...
			if (_.isEmpty(intrfccoms.intrfcId)) {
				...
				return false;
			} else if (_.isEmpty(intrfccoms.syncAsyncDscd)) {
				this.openAlert(this.text.emptySyncAsyncDscd);
				return false;
			} else if (_.isEmpty(intrfccoms.msgTrnsfrmYn)) {
				this.openAlert(this.text.emptyMsgTrnsfrmYn);
				return false;
			} else if (_.isEmpty(intrfccoms.rspsYn)) {
				this.openAlert(this.text.emptyRspsYn);
				return false;
			} else if (_.isEmpty(intrfccoms.trxTypeDscd)) {
				this.openAlert(this.text.emptyTrxTypeDscd);
				return false;
			}
			...
			for (var i = 0; i < interfaceDetailGridLength; i++) {
				...
				if (_.isEmpty(system.crgManNm)) {
					...
					return false;
				} else if (system.srTypeCd == 'RECEIVE' && _.isEmpty(system.trxCd)) {
		...
		value: function excelExport(id, multiDownload, isFirstFile, isLastFile) {
			...
			this.httpService.downloadFile('/intrfccoms/excelexport', data).then(function (res) {
				if (res.isError) {
					_this31.popupService.detailAlert(_this31.$scope, res.data.message, res.data.stackTrace);
				} else {
					...
					_this31.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
				if (multiDownload) {
					isLastFile && _this31.popupService.closeLoadingBar();
				} else {
		...
		value: function openInterfacePopup() {
			...
			this.popupService.openModal('SCR0902', { intrfcTypeCd: this.intrfcTypeCd, codes: this.codes }).then(function (intrfccom) {
				return _this32.setIntrfGridData(intrfccom.intrfcId, true);
			}).catch(function () {});
		}
	}, {
		...
		value: function _createInterFaceId() {
			...
			this.httpService.post('/intrfccoms/intrfcidcreate', requestBody).then(function (res) {
				if (res.isError) {
					_this33.popupService.detailAlert(_this33.$scope, res.data.message, res.data.stackTrace);
					return;
				}

				_this33.detail.intrfcId = res.intrfcId;
				_this33.detail.workStatusCd = 'WORKING';
				_this33.refHistoryGrid.records = [];
			});
		}
	}, {
		key: 'refreshGrid',
		...
		value: function xlsExport() {
			...
			selections.map(function (id, idx) {
				setTimeout(function () {
					_this34.excelExport(id, true, idx === 0, selectionLength === idx + 1);
				}, idx * 500);
			});
		}
	}, {
		...
		value: function exportDefinition() {
			...
			this.httpService.downloadFile(url, this.searchParam.intrfccoms, 'get').then(function (res) {
				if (res.isError) {
					_this35.popupService.detailAlert(_this35.$scope, res.data.message, res.data.stackTrace);
				} else {
					var header = decodeURIComponent(res.headers('Content-Disposition'));
					var fileName = header.split("=")[1].replace(/\"/gi, '');
					console.log(fileName);
					...
					var blob = new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
					_this35.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
				_this35.popupService.closeLoadingBar();
			});
		}
	}, {
		key: 'importDefinition',
		value: function importDefinition() {
			var _this36 = this;

			var fileList = [];
			this.httpService.uploadFileList('/intrfccoms/import/definition', { name: 'intrfcFile', file: fileList }, { intrfcTypeCd: this.intrfcTypeCd }).then(function (res) {
				console.log(res);
			}).finally(function () {
	...
	return SCR0701Controller;
}();

(0, _angular.module)(_app2.default.name).controller('SCR0701Controller', SCR0701Controller);
...
module.exports = __webpack_require__("02f34c1db395e965c161");


/***/ })

},[54]);
//# sourceMappingURL=SCR0701.controller.js.map