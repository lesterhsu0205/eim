webpackJsonp(["app\\views\\wrap\\manageEaiInterfaceDetail\\SCR0801.controller"],{
...
module.exports = __webpack_require__("c436495fce20995d0cf8");


/***/ }),

/***/ "c436495fce20995d0cf8":
/***/ (function(module, exports, __webpack_require__) {
...
var SCR0801Controller2 = function () {
	function SCR0801Controller2($scope, $state, $compile, $timeout, popupService, httpService, utilService, gridService, codeService, metaService, userService, codes) {
		var _this = this;

		(0, _classCallCheck3.default)(this, SCR0801Controller2);
	...
	(0, _createClass3.default)(SCR0801Controller2, [{
		key: 'initPrevData',
		...
		value: function initGrid() {
			...
			this.inferfaceMainGrid = {
				...
				columns: [{ field: 'intrfcId', caption: this.text.intrfcId, size: '1.5%', sortable: true }, { field: 'intrfcNm', caption: this.text.intrfcNm, size: '2.5%', sortable: true, attr: 'align=left' }, { field: 'intrfcNmSub', caption: this.text.intrfcNmSub, size: '2.5%', sortable: true, attr: 'align=left' }, {
					...
					} }, { field: 'msgTrnsfrmYn', caption: this.text.msgTrnsfrmYn, size: '0.5%', sortable: true }, { field: 'rspsYn', caption: this.text.rspsYn, size: '0.5%', sortable: true }, { field: 'trxCd', caption: this.text.trxCd, size: '0.8%', sortable: true }, {
					field: 'execEnvDscd', caption: this.text.execEnvDscd, size: '0.8%', sortable: true,
					render: function render(data) {
						return _this5.getExecEnvDscd(data.execEnvDscd);
					}
		...
		value: function setIntrfGridData(id) {
			...
			httpService.get('/intrfccoms/' + id).then(function (data) {
				...
				_this11.useRequestMsgMapping = false;
				_this11.useResponseMsgMapping = false;
		...
		value: function setDefaultDeployTarget() {
			...
			this.httpService.post('/depolysyss/getlist', ['EAI', 'SIM', 'RSH', 'E2E']).then(function (data) {
		...
		value: function openInterfaceIdCreatePopup() {
			...
			this.popupService.openModal('SCR0702', { intrfcTypeCd: this.intrfcTypeCd }).then(function (intrfc) {
				...
				_this21.changeIntrfcWay();
				_this21._changeDetailColumnByTrxDscd();
			}).catch(function () {});
		}
	}, {
		...
		key: 'getExecEnvDscd',
		value: function getExecEnvDscd(execEnvDscd) {
			switch (execEnvDscd) {
				case 'A':
					return 'All Server';
				case 'D':
		...
		value: function _deployInterface() {
			...
			this.httpService.post('/intrfccoms/deploy', param).then(function (res) {
				if (res.isError) {
					_this26.popupService.detailAlert(_this26.$scope, res.data.message, res.data.stackTrace, res.data.parameters);
					return;
				}

				_this26.setDeployResult(res.intrfcDeployResponse);
				...
				var alertText = _this26.text.completeDeploy;
				/*
    ...
    */
		...
		value: function redeployInterface(intrfcdeployhist) {
			...
			this.httpService.post('/intrfccoms/redeploy', intrfcdeployhistClone).then(function (res) {
				...
				if (res.isError) {
					_this27.popupService.detailAlert(_this27.$scope, res.data.message, res.data.stackTrace);
					return;
				}
				...
				_this27.popupService.closeLoadingBar();
				_this27.openAlert(_this27.text.completeRedeploy);
			}).finally(function () {
				_this27.popupService.closeLoadingBar();
			});
		}
		...
		value: function save() {
			...
			requestPromise.then(function (res) {
				...
				_this28.getIntrfccoms();
				if (_this28.isAdd) {
					_this28.setIntrfGridData(res.intrfcId);
				} else {
					_this28.setIntrfGridData(detail.intrfcId);
				}

				_this28.openAlert(bxMsg('common.saved'));
				_this28.offEditMode();

				// 생성 수정시, msgLayoutTranYn를 N으로 변경 
				_this28.detail.msgLayoutTranYn = 'N';
			});
		...
		value: function excelExport(id, multiDownload, isFirstFile, isLastFile) {
			...
			this.httpService.downloadFile('/intrfccoms/excelexport', data).then(function (res) {
				if (res.isError) {
					...
					_this29.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
				if (multiDownload) {
					isLastFile && _this29.popupService.closeLoadingBar();
				} else {
		...
		value: function openInterfacePopup() {
			...
			this.popupService.openModal('SCR0902', { intrfcTypeCd: this.intrfcTypeCd, codes: this.codes }).then(function (intrfccom) {
				return _this30.setIntrfGridData(intrfccom.intrfcId, true);
			}).catch(function () {});
		}
	}, {
		...
		value: function _createInterFaceId() {
			...
			this.httpService.post('/intrfccoms/intrfcidcreate', requestBody).then(function (res) {
				...
				_this31.detail.intrfcId = res.intrfcId;
				_this31.detail.workStatusCd = 'WORKING';
				_this31.refHistoryGrid.records = [];
			});
		}
	}, {
		key: 'changeIntrfcWay',
		...
		value: function xlsExport() {
			...
			selections.map(function (id, idx) {
				setTimeout(function () {
					_this32.excelExport(id, true, idx === 0, selectionLength === idx + 1);
				}, idx * 500);
			});
		}
	}, {
		...
		value: function exportDefinition() {
			...
			this.httpService.downloadFile(url, this.searchParam.intrfccoms, 'get').then(function (res) {
				if (res.isError) {
					...
					var blob = new Blob([res.data], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
					_this33.utilService.saveFile(blob, fileName);
				}
			}).finally(function () {
	...
	return SCR0801Controller2;
}();

(0, _angular.module)(_app2.default.name).controller('SCR0801Controller2', SCR0801Controller2);
