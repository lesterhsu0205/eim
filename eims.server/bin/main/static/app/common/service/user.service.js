...
var UserService = function () {
	...
	(0, _createClass3.default)(UserService, [{
		...
		key: 'requestSSO',
		value: function requestSSO() {
			var _this3 = this;

			var defer = this.$q.defer();

			this.httpService.post('/sso').then(function (res) {
				if (res.isError) defer.reject(res);else defer.resolve(_this3._setUser(res));
			});
			return defer.promise;
		}
	}, {
		...
		value: function requestLogout(scope) {
			...
			this.popupService.simpleConfirm(scope, bxMsg('common.logoutMsg'), function () {
				return _this5.doLogout();
  ...
  */
		...
		value: function _setUser(loginUserInfo) {
			...
			loginUserInfo.permList.map(function (perm) {
				_this6._user.perm[perm.permId] = true;
			});
