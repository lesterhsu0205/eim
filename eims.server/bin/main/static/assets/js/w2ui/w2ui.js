                ...
                case 67: // - c
                    ...
                    if (event.metaKey || event.ctrlKey) {
                        if (obj.last.isSafari) {
                            ...
                            obj.last.copy_event = obj.copy(false, event);
                            if(obj.last.copy_event.text.indexOf('&nbsp;') != -1){
                            	obj.last.copy_event.text = obj.last.copy_event.text.replace(/&nbsp;/gi,'');
                            }
                            $('#grid_'+ obj.name + '_focus').val(obj.last.copy_event.text).select();
                            obj.copy(obj.last.copy_event, event);
                        }
        ...
        refreshBody: function () {
            ...
            gridBody.data('scrolldata', { lastTime: 0, lastDelta: 0, time: 0 })
                ...
                .on("mousewheel DOMMouseScroll ", function(event) {
                    ...
                    if(recordsContainer.get(0).scrollHeight > recordsContainer.get(0).clientHeight) {
                    	event.preventDefault();
                	}
            ...
            if ($('.w2ui-layout').length === 0) { // if there is layout, it will send a resize event
                $(window).off('resize.w2ui-'+ this.name)
                    .on('resize.w2ui-'+ this.name, function (event) {
                    	w2ui[obj.name] && w2ui[obj.name].resize();
                    });
            }
            return (new Date()).getTime() - time;
...
(function ($) {
    ...
    w2form.prototype = {
        ...
        render: function (box) {
            ...
            // attach to resize event
            if ($('.w2ui-layout').length === 0) { // if there is layout, it will send a resize event
                this.tmp_resize = function (event) { w2ui[obj.name] && w2ui[obj.name].resize(); };
