...
var SCR0201Controller = function () {
	...
	(0, _createClass3.default)(SCR0201Controller, [{
		...
		value: function initGrid() {
			...
			this.roleOptions = {
				...
				onClick: function onClick(e) {
					...
					_this3.getPermList(editData.roleId);
				}
			};
		...
		key: 'getPermList',
		value: function getPermList(roleId) {
			var _this7 = this;

			this.httpService.get('/roles/' + roleId + '/perms').then(function (data) {
		...
		value: function deletePerm() {
			...
			this.httpService.delete('/roles/' + roleId + '/perms/' + permId).then(function (data) {
				if (data.isError) return;
				_this10.getPermList(roleId);
			});
		}
	}, {
		key: 'search',
		...
		value: function updatePerms(records) {
			...
			this.httpService.put('/roles/' + roleId + '/perms', records).then(function (res) {
				if (res.isError) {
					_this15.popupService.detailAlert(_this15.$scope, res.data.message, res.data.stackTrace);
					return;
				}

				_this15.getPermList(roleId);
