...
class SCR0701Controller2 {
	...
	editIntrfcDetailGrid(isEdit = true){
		this.interfaceDetailGrid.columns = [
			...
			{ field: 'trxCd', caption: '<i class="asterisk"></i>' + this.text.trxCd, size: '1.5%',editable: {type:'text'} }
		];
	}
	...
	setDefaultDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['MCA', 'MCI_E', 'SIM', 'RSH', 'E2E']).then(data => {
			data.map((deploySys) => {
...
module(App.name).controller('SCR0701Controller2', SCR0701Controller2);