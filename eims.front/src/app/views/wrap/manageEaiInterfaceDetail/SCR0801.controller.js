...
class SCR0801Controller2 {
	...
	setDefaultDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['EAI', 'SIM', 'RSH', 'E2E']).then(data => {
			data.map((deploySys) => {
	...
	_checkValid(){
		...
		for(let i = 0; i < interfaceDetailGridLength; i++){
			...
			if(_.isEmpty(system.crgManNm)) {
				...
				return false;
			}else if(intrfccoms.trxDscd === 'ONLINE' && 
					intrfccoms.intrfcWayCd == 'APTOAP' && 
					system.srTypeCd == 'RECEIVE' && 
					_.isEmpty(system.trxCd)){
...
module(App.name).controller('SCR0801Controller2', SCR0801Controller2);