	...
	initGrid() {
		...
		this.inferfaceMainGrid = {
			...
			columns: [
				...
				{ field: 'instCd', caption: this.text.instCd, size: this.user.locale === 'en'? '100px' : '0.5%', sortable: true,},
				{ 
...
//			});
	...
	setFEBOnlineDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['CBK', 'FEB']).then(data => {
			data.map((deploySys) => {
	...
	setFEBBatchDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['CBK', 'FEB', 'EAI']).then(data => {
			data.map((deploySys) => {
				 deploySys.deployUrl = deploySys.deploySysUrl;
				 delete deploySys.deploySysUrl;
			});
	...
	setFEPDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['CBK']).then(data => {
			data.map((deploySys) => {
				 deploySys.deployUrl = deploySys.deploySysUrl;
				 delete deploySys.deploySysUrl;
			});
						 ...
						 .then(intrfc => {
							...
							this.editIntrfcDetailGrid();
							 ...
							 if(sndSys.sysCd == 'FEB' || rcvSys.sysCd  == 'FEB') {
								if(detail.trxDscd == 'ONLINE'){
									this.setFEBOnlineDeployTarget();
							 	} else {
									this.setFEBBatchDeployTarget();									 
								 }
							 } else {
								this.setFEPDeployTarget()
							 }
						 })
						 .catch(()=>{});
	}
	...
	_checkValid(){
		...
		const sndReqLayoutGridRecords = w2ui[this.sndReqLayoutGrid.name].records;
		const sndResLayoutGridRecords = w2ui[this.sndResLayoutGrid.name].records;
		const rcvReqLayoutGridRecords = w2ui[this.rcvReqLayoutGrid.name].records;
		const rcvResLayoutGridRecords = w2ui[this.rcvResLayoutGrid.name].records;
...
module(App.name).controller('SCR0901Controller', SCR0901Controller);