...
class SCR0801Controller {
	...
	_changeDetailColumnByTrxDscd(){
		const columns = this.interfaceDetailGrid.columns;
	...
	setIntrfAccordion(data, isCreateInterfaceId = false) {
		...
		this.$timeout(() => {
			w2ui[this.sndReqLayoutGrid.name].refresh();
			w2ui[this.sndResLayoutGrid.name].refresh();
			w2ui[this.rcvReqLayoutGrid.name].refresh();
			w2ui[this.rcvResLayoutGrid.name].refresh();
		});
	...
	setDefaultDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['EAI', 'CBK']).then(data => {
			data.map((deploySys) => {
	...
	setCssDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['EAI', 'CBK','CSS']).then(data => {
			data.map((deploySys) => {
				 deploySys.deployUrl = deploySys.deploySysUrl;
				 delete deploySys.deploySysUrl;
			});
	...
	_onEdit(){
		const $forms = $(`#searchWrap_detail,
						...
						.find('input,select');
		const $textareas = $(`#searchWrap_dbDesc`)
						.find('div:not(.disabled)')
						.find('textarea');
	...
	_checkValid(){
		...
		if(intrfccoms.trxDscd == 'BATCH' && intrfccoms.intrfcWayCd == 'DBTODB') {
			...
			if(Number.isNaN(Number(intrfccoms.eaiDto.scheduleProcInterval))) {
	...
	changeIntrfcWay(){
		...
		case 'ONLINE':
			this.interfaceWayList = this.codes['INTRFC_WAY_CD'].filter(v => v.cdVal === 'APTOAP');
			break;
		case 'BATCH':
			this.interfaceWayList = this.codes['INTRFC_WAY_CD'].filter(v => v.cdVal === 'FILETOFILE');
			break;
		default:
...
module(App.name).controller('SCR0801Controller', SCR0801Controller);