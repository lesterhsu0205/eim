...
class SCR0701Controller {
				...
				if (eTarget.localName === 'button') {
					...
					case 'redeploy':
						this.popupService.simpleConfirm(this.$scope, this.text.confirmRedeploy,
								()=>this.redeployInterface(grid.get(recid)));
						break;
					}
				}
			},
	...
	editIntrfcDetailGrid(isEdit = true){
		this.interfaceDetailGrid.columns = [
			...
			{ field: 'trxCd', caption: '<i class=""></i>' + this.text.trxCd, size: '1.5%',editable: {type:'text'} }
		];
	}
	...
	setIntrfAccordion(data, isCreateInterfaceId = false){
		const { intrfcsrsysdtDto : intrfcsrsysRecords, 
			...
			intrfcdeployhisthsDto : intrfcdeployhisthsDtoRecords } = data;
	...
	setDefaultDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['MCA', 'CBK', 'SUI']).then(data => {
			data.map((deploySys) => {
	...
	setCssDeployTarget() {
		this.httpService.post(`/depolysyss/getlist`, ['MCA', 'CBK', 'SUI','CSS']).then(data => {
			data.map((deploySys) => {
				 deploySys.deployUrl = deploySys.deploySysUrl;
				 delete deploySys.deploySysUrl;
			});
	...
	openSysCdSPopup(){
		this.popupService.openModal('SCR1302')
						 .then(sys => this.searchParam.intrfccoms.sysCdS = sys.sysCd)
						 .catch(()=>{});
	}
	
	openSysCdRPopup(){
		this.popupService.openModal('SCR1302')
						 .then(sys => this.searchParam.intrfccoms.sysCdR = sys.sysCd)
						 .catch(()=>{});
	}
	...
	openDeploySystemPopup(){
		...
		this.popupService.openModal('SCR1202', {deploySysGrpCd : this.intrfcTypeCd, codes: this.codes })
						 .then(deploySys => {
	...
	addMsgLayoutToSndRcvMsg(optionName){
		if(!this.isEdit) return;
		const grid = w2ui[this[optionName].name];
		const sndReqLayoutGrid = w2ui[this['sndReqLayoutGrid'].name];
		const sndResLayoutGrid = w2ui[this['sndResLayoutGrid'].name];
							 ...
							 msglayoutbsDtoList.map((msglayoutbsDto) => {
								...
								if(optionName === 'sndReqLayoutGrid') {
								  if(!_.isEmpty(sndResLayoutGrid.get(msglayoutbsDto.msgLayoutId))){
								    this.openAlert(this.text.systemDuplicateMsg);
									return;
								  }
	...
	deployAllInterface(event){
		this.popupService.simpleConfirm(this.$scope,
				this.text.confirmAllDeploy,
				()=>this._deployAllInterface());
	}
	...
	_deployAllInterface(){
		...
		this.httpService.post('/intrfccoms/deployAll')
			.then((res)=>{
				if(res.isError){
	...
	save(){
		...
		const sendMsgMapTrgGridRecords = _w2ui[this.sendMsgMapTrgGrid.name].records;
		const rcvMsgMapTrgGridRecords = _w2ui[this.rcvMsgMapTrgGrid.name].records;
	...
	_checkValid(){
		...
		const sendMsgMapTrgGridRecords = w2ui[this.sendMsgMapTrgGrid.name];
		const rcvMsgMapTrgGridRecords = w2ui[this.rcvMsgMapTrgGrid.name];
		...
		if (intrfccoms.msgTrnsfrmYn == 'Y') {
			for(let i = 0; i < sendMsgMapTrgGridRecords.records.length; i++){
				const senmdMappingGrid = sendMsgMapTrgGridRecords.records[i];
			...
			for(let i = 0; i < rcvMsgMapTrgGridRecords.records.length; i++){
				const revdMappingGrid = rcvMsgMapTrgGridRecords.records[i];
	...
	importDefinition() {				
		let fileList = [];
		this.httpService.uploadFileList('/intrfccoms/import/definition', { name:'intrfcFile', file: fileList }, {intrfcTypeCd: this.intrfcTypeCd})
				 .then(res => {
					 console.log(res);	 			
				 })
				 .finally(() => {
...
module(App.name).controller('SCR0701Controller', SCR0701Controller);