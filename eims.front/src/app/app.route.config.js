...
const routeConfig = [
	{
		...
		cache: false,
		resolve: {
			bxMsg: (messageService) => messageService.initLogin(['login', 'menu'])
		},
	},
	{
		state: 'main',
		...
		cache: false,
		resolve: {
			bxMsg: (messageService) => messageService.init(['login', 'menu', 'code', 'common', 'manageApp', 'manageMsg', 
				'mangePermission', 'manageCommSystem', 'manageTrxCode', 'manageExtInstCode', 
				'manageUser','manageRole','manageDeploySystem','manageMetaInfo',
				'manageActionHistory', 'manageMciInterface', 'manageEaiInterface', 'manageFepInterface'])
		},
		children: [
			{
				state: 'main.blank',
			...
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE','TRAN_DSCD','MSG_TYPE',
						'CHN_DSCD', 'DATA_FORMAT_CD', 'ALIGN_CD', 'FILLER_CD', 'PARAM_TYPE', 'PRIVACY_CD', 'YN_CD', 'DATA_TYPE', 'CHL_DSCD', 'WORK_STATUS_CD'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, [
						'GRID_PAGE_SIZE','TRAN_DSCD', 'SYNC_DSCD', 'YN_CD', 'DUP_FILE_PROC_CD', 'RECORD_SEPARATOR_CD',
						'WORK_STATUS_CD','SENC_RECV_DSCD', 'GEN_CYCLE_CD', 'REQ_EVENT_DSCD', 'TIMEOUT_PROC_CD', 'DEPLOY_SYS_DSCD',
						'MAPPING_DSCD','MSG_TYPE', 'CHL_DSCD', 'BACK_APR_STAT', 'TRX_TYPE_DSCD'])
				}
			},
			{
			...
			},
			{
				...
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, [
						'GRID_PAGE_SIZE','TRAN_DSCD', 'SYNC_DSCD', 'YN_CD', 'DUP_FILE_PROC_CD', 'RECORD_SEPARATOR_CD', 
						'WORK_STATUS_CD','SENC_RECV_DSCD', 'GEN_CYCLE_CD', 'REQ_EVENT_DSCD', 'TIMEOUT_PROC_CD', 'INTRFC_WAY_CD', 
						'MAPPING_DSCD', 'DEPLOY_SYS_DSCD', 'MSG_TYPE', 'CHL_DSCD', 'BACK_APR_STAT', 
						'RECV_DB_ACT_TYPE', 'TRAN_POST_PROC', 'TRAN_BEFORE_SCRIPT_TYPE', 'TRAN_POST_SCRIPT_TYPE', 'FIELD_ENCODING', 'WIDE_HALF_CHAR_CD'])
				}
			},
			{
				...
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, [
						'GRID_PAGE_SIZE','TRAN_DSCD', 'SYNC_DSCD', 'YN_CD', 'DUP_FILE_PROC_CD', 'RECORD_SEPARATOR_CD',
						'WORK_STATUS_CD','SENC_RECV_DSCD', 'GEN_CYCLE_CD', 'REQ_EVENT_DSCD', 'TIMEOUT_PROC_CD', 'INTRFC_WAY_CD', 
						'MAPPING_DSCD', 'DEPLOY_SYS_DSCD', 'MSG_TYPE', 'CHL_DSCD', 'BACK_APR_STAT', 'RECV_DB_ACT_TYPE', 
						'TRAN_POST_PROC', 'TRAN_BEFORE_SCRIPT_TYPE', 'TRAN_POST_SCRIPT_TYPE', 'FIELD_ENCODING', 'WIDE_HALF_CHAR_CD'])
				}
			},
			{
				...
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, [
						'GRID_PAGE_SIZE','TRAN_DSCD', 'SYNC_DSCD', 'YN_CD', 'DUP_FILE_PROC_CD', 'RECORD_SEPARATOR_CD', 
						'WORK_STATUS_CD','SENC_RECV_DSCD', 'GEN_CYCLE_CD', 'REQ_EVENT_DSCD', 'TIMEOUT_PROC_CD', 'DEPLOY_SYS_DSCD',
						'MAPPING_DSCD', 'MSG_TYPE', 'CHL_DSCD', 'BACK_APR_STAT', 'SYNC_DSCD_FEP', 'PROTOCOL_DSCD', 'HTTP_METHOD',
						'WIDE_HALF_CHAR_CD', 'ENC_CD_MAPPING', 'FIELD_ENCODING_ONL'])
				}
			},
			{
				...
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, [
						'GRID_PAGE_SIZE','TRAN_DSCD', 'SYNC_DSCD', 'YN_CD', 'DUP_FILE_PROC_CD', 'RECORD_SEPARATOR_CD',
						'WORK_STATUS_CD','SENC_RECV_DSCD', 'GEN_CYCLE_CD', 'REQ_EVENT_DSCD', 'TIMEOUT_PROC_CD', 'DEPLOY_SYS_DSCD',
						'MAPPING_DSCD', 'MSG_TYPE', 'CHL_DSCD', 'BACK_APR_STAT', 'SYNC_DSCD_FEP',
						'WIDE_HALF_CHAR_CD', 'ENC_CD_MAPPING', 'FIELD_ENCODING_ONL'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE','DEPLOY_SYS_DSCD'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE','YN_CD'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE', 'LEVEL_CD'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE', 'EXT_INST_TYPE'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE'])
				}
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE', 'ACTION_STAT_CD', 'HISTORY_DSCD'])
				}
			},
			{
			...
			},
			{
				...
				cache: false,
				resolve: {
					codes: ($q, codeService) => _commonCode($q, codeService, ['GRID_PAGE_SIZE', 'PERM_TYPE'])
				}
			}
		]
	}
...
function _commonCode($q, codeService, codeIds){
	...
	codeService.getCodesList(codeIds)
		.then(data => {
			for(let i in codeIds) {
...
(function flatRouteConfig(config) {
	...
	config.map(v => {
		const {state, url, templateUrl, controller, controllerUrl, injectUrl, abstract, cache, children, resolve} = v;
